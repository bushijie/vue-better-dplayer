<template>
  <div id="app">
    ...
    <div style="display: flex;flex-direction: row;flex-wrap: wrap;">
      ...
      <button @click="addFlvVideo">添加一个FLV视频</button>
      <button @click="delVideo">删除第一个视频</button>
      <button @click="addVideos">添加多个</button>
    </div>
    <div style="display: flex;flex-direction: row;flex-wrap: wrap;">
      <template v-for="(item, index) in items">
        <vue-better-dplayer v-bind:key="item.objectId" :model="item" style="width: 270px;height: 480px;"
                            @hlsError="networkError" @flvError="flvError" :options="getOptions(item)"></vue-better-dplayer>
      </template>
    </div>
  </div>
</template>
...
<script>
    export default {
        ...
        methods: {
            ...
            flvError(event,model){
                console.log('event:  ' + event+'  objectId:  ' + model.objectId)
            ...
            addFlvVideo() {
                this.items.push({
                    objectId: this.getObjectId(),
                    videoUrl: 'http://alplay.nodemedia.cn/live/cam_101.flv',
                    type: 'betterFlv'
                })
            },
            addErrorVideo() {
